...
def remove_ind(A: sparse, ind_real: np.array, ind: np.array) -> tuple[sparse, list]:
    ...
    return  A_k(sparse):sparse matrix containing only the corresponding index of ind
            ind: list of the corresponding indices of the matrix to the volume
    """
...
def compute_fully_connected(A: sparse, ind: list) -> tuple[sparse, list]:
    """Check is an sparse matrix is fully connected
    Args:   A(np.sparse): sparse Adjacent matrix
            ind(list): list of the corresponding indices of the matrix to the volume
...
def compute_A_wm(A: sparse, patient_id: int) -> tuple[sparse, list]:
...
def compute_binary_matrix(A: sparse, thresold: float, ind: list) -> tuple[sparse, list]:
...
def compute_D(A: sparse, ind: list) -> tuple[sparse, list]:
...
def compute_L(
    A: sparse, ind: list, path_matrix: str, save: bool
) -> tuple[sparse, list]:
...
def compute_eigenvalues(
    L: sparse, k: int, ind: list, path_matrix: str, save: bool
...
def is_symetric(A: sparse) -> bool:
    ...
    return sk.validation.check_symmetric(A)
...
def compute_nift(
    ...
    Args:   path_nifti_in(str):path to the src nifti file
            ...
            indices_raw(list):list of the raw indices
    """
